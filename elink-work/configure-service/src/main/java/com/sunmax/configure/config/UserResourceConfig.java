...
import com.sunmax.configure.service.feign.SauthService;
...
/**
 * configure-service 安全配置
 * 替代原 SecurityConfig（permitAll 全放行），接入 RedisTokenAuthenticationFilter 鉴权
 */
@Slf4j
@Configuration
@EnableWebSecurity
...
public class UserResourceConfig {
    ...
    /**
     * 权限校验：根据 userAccount + clientId 查询权限列表，匹配请求 URI
     */
    public boolean canAccess(HttpServletRequest request, Authentication authentication) {
        ...
        try {
            ...
            List<String> urls = permissionList.stream()
                    .filter(p -> Objects.equals(p.getType(), 2))
                    .map(PermissionInfoListDto::getUrl)
    ...
    /**
     * 从 Authentication 中获取 clientId
     * 兼容 Redis Token 认证（details 为 JSONObject）
     */
