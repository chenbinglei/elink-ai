...
public class HMacMD5Util {
    ...
    public static String getSeq(String timeStamp) {
        ...
        try {
            if (TIME_STAMP == null) {
                ...
                if (Objects.equals(TIME_STAMP, timeStamp)) {
                    SEQ = String.format("%04d", Long.parseLong(SEQ) + 1);
                    return SEQ;
                } else {
                    TIME_STAMP = timeStamp;
                    SEQ = "0001";
     ...
     * 数据拼接
     */
    public static String dataSplice(String data, String timeStamp, String seq) {
        return PlatformConfig.PLATFORM_ID + data + timeStamp + seq;
    }
     ...
     * @return 加密结果
     */
    public static String getHmacMd5HexString(String key, String data) {
        try {
            ...
            byte[] actualKey = byteKey; // Actual key.
             ...
             */
            byte[] firstAppendResult = new byte[kIpadXorResult.length + byteData.length];
            System.arraycopy(kIpadXorResult, 0, firstAppendResult, 0, kIpadXorResult.length);
            System.arraycopy(byteData, 0, firstAppendResult, keyArr.length, byteData.length);
