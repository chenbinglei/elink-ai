...
public class DateUtil {
    ...
    public static LocalDateTime wxStrToLocalDateTime(String dateStr) {
        String dataStr = dateStr.substring(0, 4) + "-" + dateStr.substring(5, 7) + "-" + dateStr.substring(8, 10) + FileUtil.SPACE
                + dateStr.substring(11, 13) + ":" + dateStr.substring(14, 16) + ":" + dateStr.substring(17, 19);
        return strToLocalDateTime(dataStr);
    }
    ...
    public static String iso8601ToStr(String iso8601) {
        try {
            LocalDateTime dateTime = OffsetDateTime.parse(iso8601).toLocalDateTime();
            return dateTime.format(DateTimeFormatter.ofPattern(DATE_TIME_PATTERN));
        } catch (Exception e) {
            throw new IllegalArgumentException("无效的 ISO8601 日期格式", e);
        }
    }
    ...
    public static List<String> getDateBetweenMinutes(LocalDateTime startTime, LocalDateTime endTime, Integer interval) {
        ...
        for (LocalDateTime dateTime = startTime; dateTime.isBefore(endTime.plusMinutes(interval)); dateTime = dateTime.plusMinutes(interval)) {
            list.add(DateUtil.localDateTimeToStr(dateTime));
        }
    ...
    public static List<String> getDateBetweenMonth(LocalDateTime startTime, LocalDateTime endTime, Integer interval) {
        List<String> list = new ArrayList<>();
        for (LocalDateTime dateTime = startTime; dateTime.isBefore(endTime.plusMonths(interval)); dateTime = dateTime.plusMonths(interval)) {
            list.add(DateUtil.localDateTimeToStr(dateTime));
        }
        return list.stream().filter(d -> !strToLocalDateTime(d).isAfter(endTime)).collect(Collectors.toList());
    }
    ...
    public static List<String> getDateBetweenSeconds(LocalDateTime startTime, LocalDateTime endTime, Integer interval) {
        List<String> list = new ArrayList<>();
        for (LocalDateTime dateTime = startTime; dateTime.isBefore(endTime.plusSeconds(interval)); dateTime = dateTime.plusSeconds(interval)) {
            list.add(DateUtil.localDateTimeToStr(dateTime));
        }
        return list.stream().filter(d -> !strToLocalDateTime(d).isAfter(endTime)).collect(Collectors.toList());
    }
    ...
    public static List<String> getDateBetweenMinutes(LocalDateTime startTime, LocalDateTime endTime) {
        List<String> list = new ArrayList<>();
        for (LocalDateTime dateTime = startTime; dateTime.isBefore(endTime.plusMinutes(5L)); dateTime = dateTime.plusMinutes(5L)) {
            list.add(DateUtil.localDateTimeToStr(dateTime));
        }
        return list.stream().filter(d -> !strToLocalDateTime(d).isAfter(endTime)).collect(Collectors.toList());
    }
     ...
     * @return 相差时间 天数
     */
    public static Long compareDiffBetweenDays(LocalDateTime startDate, LocalDateTime endDate) {
...
//        }
    ...
    public static List<String> getDateBetween(Integer dateType, String startDate, String endDate) {
        ...
        LocalDate endLocalDate = DateUtil.strToLocalDate(endDate);
        switch (dateType) {
            case 1: //日
                resultList.add(startDate);
                for (int i = 0; i < ChronoUnit.DAYS.between(DateUtil.strToLocalDate(startDate), endLocalDate); i++) {
                    resultList.add(DateUtil.localDateToStr(DateUtil.strToLocalDate(startDate).plusDays(i + 1)));
                }
                break;
            case 2: //月
                LocalDate monthStartLocalDate = DateUtil.strToLocalDate(startDate).with(TemporalAdjusters.firstDayOfMonth());
                ...
                break;
            case 3: //年
                LocalDate dayStartLocalDate = DateUtil.strToLocalDate(startDate).with(TemporalAdjusters.firstDayOfYear());
    ...
    /**
     * 获取两个日期之间的所有日期（自定义） 海天项目使用
     *
     * @param dateType  日期类型 1-日 2-月 3-年
     * @param startTime 开始时间
     * @param endTime   结束时间
     * @return 日-(MM/dd) 月-(yyyy/MM) 年-(yyyy)
     */
    public static List<String> getCustomDateBetween(Integer dateType, String startTime, String endTime) {
        List<String> resultList = Lists.newArrayList();
        LocalDate endLocalDate = DateUtil.strToLocalDateTime(endTime).toLocalDate();
        switch (dateType) {
            case 1: //日
                LocalDate dayStartLocalDate = DateUtil.strToLocalDateTime(startTime).toLocalDate();
                resultList.add(dayStartLocalDate.format(DateTimeFormatter.ofPattern("MM/dd")));
                for (int i = 0; i < ChronoUnit.DAYS.between(dayStartLocalDate, endLocalDate); i++) {
                    resultList.add(dayStartLocalDate.plusDays(i + 1).format(DateTimeFormatter.ofPattern("MM/dd")));
                }
                break;
            case 2: //月
                LocalDate monthStartLocalDate = DateUtil.strToLocalDateTime(startTime).toLocalDate().with(TemporalAdjusters.firstDayOfMonth());
                resultList.add(monthStartLocalDate.format(DateTimeFormatter.ofPattern("yyyy/MM")));
                for (int i = 0; i < ChronoUnit.MONTHS.between(monthStartLocalDate, endLocalDate); i++) {
                    resultList.add(monthStartLocalDate.plusMonths(i + 1).format(DateTimeFormatter.ofPattern("yyyy/MM")));
                }
                break;
            case 3: //年
                LocalDate yearStartLocalDate = DateUtil.strToLocalDateTime(startTime).toLocalDate().with(TemporalAdjusters.firstDayOfYear());
                resultList.add(yearStartLocalDate.format(DateTimeFormatter.ofPattern("yyyy")));
                for (int i = 0; i < ChronoUnit.YEARS.between(yearStartLocalDate, endLocalDate); i++) {
                    resultList.add(yearStartLocalDate.plusYears(i + 1).format(DateTimeFormatter.ofPattern("yyyy")));
                }
                break;
        }
        return resultList;
    ...
    public static List<LocalDate> getMonthDayByType(int year, int month, int type) {
        ...
        while (cal.get(Calendar.YEAR) == year && cal.get(Calendar.MONTH) < month) {
            ...
            if (type == 1) { //全时段
                resultList.add(DateUtil.dateToLocalDateTime(cal.getTime()).toLocalDate());
            } else if (type == 2) { //工作日
    ...
    public static List<LocalDateTime> getLocalDateTimeBetween(LocalDateTime startTime, LocalDateTime endTime, String timeInterval) {
        List<LocalDateTime> dateList = Lists.newArrayList();

        if (timeInterval.contains(StaticParamVo.S)) { //秒
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.S, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusSeconds(interval)) {
                dateList.add(time.withSecond(0));
            }
        }else if (timeInterval.contains(StaticParamVo.M)) { //分
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.M, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusMinutes(interval)) {
                dateList.add(time.withSecond(0));
            }
        } else if (timeInterval.contains(StaticParamVo.H)) {
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.H, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusHours(interval)) {
                dateList.add(time.withMinute(0).withSecond(0));
            }

        } else if (timeInterval.contains(StaticParamVo.D)) { //日
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.D, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusDays(interval)) {
                dateList.add(time.withHour(0).withMinute(0).withSecond(0));
            }

        } else if (timeInterval.contains(StaticParamVo.W)) { //周
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.W, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusWeeks(interval)) {
                dateList.add(time.withHour(0).withMinute(0).withSecond(0));
            }
        } else if (timeInterval.contains(StaticParamVo.N)) { //月
            startTime = startTime.with(TemporalAdjusters.firstDayOfMonth());
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.N, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusMonths(interval)) {
                dateList.add(time.withHour(0).withMinute(0).withSecond(0));
            }
        } else if (timeInterval.contains(StaticParamVo.Y)) { //年
            startTime = startTime.with(TemporalAdjusters.firstDayOfYear());
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.Y, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusYears(interval)) {
                dateList.add(time.withHour(0).withMinute(0).withSecond(0));
            }
        }
        return dateList;
     ...
     * @param type 类型
     **/
    public static String getIntervalTime(LocalDateTime time, Integer interval, String type) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(localDateTimeToDate(time));
        switch (type) {
            case StaticParamVo.M:
                calendar.add(Calendar.MINUTE, -calendar.get(Calendar.MINUTE) % interval);
            break;
            case StaticParamVo.H:
                calendar.add(Calendar.HOUR_OF_DAY, -calendar.get(Calendar.HOUR_OF_DAY) % interval);
            break;
            case StaticParamVo.D:
                calendar.add(Calendar.DAY_OF_MONTH, -calendar.get(Calendar.DAY_OF_MONTH) % interval);
            break;
            case StaticParamVo.W:
                calendar.add(Calendar.WEEK_OF_YEAR, -calendar.get(Calendar.WEEK_OF_YEAR) % interval);
            break;
            case StaticParamVo.N:
                calendar.add(Calendar.MONTH, -calendar.get(Calendar.MONTH) % interval);
            break;
            case StaticParamVo.Y:
                calendar.add(Calendar.YEAR, -calendar.get(Calendar.YEAR) % interval);
            break;
        }
        calendar.set(Calendar.SECOND, 0);
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    ...
    public static List<LocalDateTime> getLocalDateTimeBetweenAuf(LocalDateTime startTime, LocalDateTime endTime, String timeInterval) {
        List<LocalDateTime> dateList = Lists.newArrayList();

        if (timeInterval.contains(StaticParamVo.M)) { //分
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.M, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusMinutes(interval)) {
                LocalDateTime dateTime = time.withSecond(0).withNano(0);
                dateList.add(DateUtil.strToLocalDateTime(getIntervalTime(dateTime, interval, StaticParamVo.M)));
            }
        } else if (timeInterval.contains(StaticParamVo.H)) {
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.H, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusHours(interval)) {
                LocalDateTime dateTime = time.withMinute(0).withSecond(0).withNano(0);
                dateList.add(DateUtil.strToLocalDateTime(getIntervalTime(dateTime, interval, StaticParamVo.H)));
            }

        } else if (timeInterval.contains(StaticParamVo.D)) { //日
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.D, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusDays(interval)) {
                LocalDateTime dateTime = time.withHour(0).withMinute(0).withSecond(0).withNano(0);
                dateList.add(DateUtil.strToLocalDateTime(getIntervalTime(dateTime, interval, StaticParamVo.D)));
            }
            ...
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusWeeks(interval)) {
                LocalDateTime dateTime = time.withHour(0).withMinute(0).withSecond(0).withNano(0);
                dateList.add(DateUtil.strToLocalDateTime(getIntervalTime(dateTime, interval, StaticParamVo.W)));
            }
        } else if (timeInterval.contains(StaticParamVo.N)) { //月
            int interval = Integer.parseInt(timeInterval.replace(StaticParamVo.N, FileUtil.separator));
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusMonths(interval)) {
            ...
            for (LocalDateTime time = startTime; time.isBefore(endTime); time = time.plusYears(interval)) {
                LocalDateTime dateTime = time.withHour(0).withMinute(0).withSecond(0).withNano(0);
                dateList.add(DateUtil.strToLocalDateTime(getIntervalTime(dateTime, interval, StaticParamVo.Y)));
            }
        }
        return dateList;
    }
     ...
     * @param startTime 开始时间
     * @param endTime 结束时间
     * @param timeInterval 数据时间间隔 s-秒 m-分钟 h-小时 d-天 n-月 y-年
     * @param formatInterval 时间格式间隔 1-(yyyy-MM-dd HH:mm:ss) 2-(yyyy-MM-dd) 3-(yyyy-MM) 4-(yyyy) 5-(HH:mm:ss) 6-(HH:mm)
     * @return 时间列表
     */
    public static Set<String> getDateFormatTimeList(String startTime, String endTime, String timeInterval, Integer formatInterval) {
        Set<String> resultSet = Sets.newTreeSet();
        List<LocalDateTime> localDateTimeList = getLocalDateTimeBetween(strToLocalDateTime(startTime), strToLocalDateTime(endTime), timeInterval);
        if (CollectionUtils.isNotEmpty(localDateTimeList)) {
            resultSet = new TreeSet<>(localDateTimeList.stream().collect(Collectors.groupingBy(d -> DateUtil.format(DateUtil.localDateTimeToDate(d),
                    getFormatIntervalPattern(formatInterval)))).keySet());
        }
        return resultSet;
    }
