...
//@Slf4j
//@Configuration
//public class InterMqttDataOldHandler {
//
//    /**
...
//                        rateRes.setCTimeFrameRate(rateReqPublicVo.getCTimeFrameRate().stream().map(InterMqttDataOldHandler::apply).sorted(Comparator.comparing(CostFormat::getStartTime)).collect(Collectors.toList()));
//                    }
...
//                        rateRes.setDTimeFrameRate(rateReqPublicVo.getDTimeFrameRate().stream().map(InterMqttDataOldHandler::apply).sorted(Comparator.comparing(CostFormat::getStartTime)).collect(Collectors.toList()));
//                    }
//                    //费率响应数据发送
...
//            Map<String, Set<String>> updateAlarmRecordMap = Maps.newConcurrentMap();
//            Set<String> pileFaultCodes = Sets.newHashSet();//电桩最新故障状态码
...
//                        pileFaultCodes.addAll(FaultParseUtil.parseEmergcyFault(chargerRunFault.getChargerStopFault()).stream()
//                                .map(String::valueOf).collect(Collectors.toList()));//急停故障
//                    }
//                    if (StringUtil.isNotEmpty(chargerRunFault.getChargerComFault()) && chargerRunFault.getChargerComFault() >= 0) {
//                        pileFaultCodes.addAll(FaultParseUtil.parseGeneralFault(chargerRunFault.getChargerComFault()).stream()
//                                .map(String::valueOf).collect(Collectors.toList()));//通用故障
//                    }
//                    if (StringUtil.isNotEmpty(chargerRunFault.getTcuReportFault()) && chargerRunFault.getTcuReportFault() >= 0) {
//                        pileFaultCodes.addAll(FaultParseUtil.parseTcuFault(chargerRunFault.getTcuReportFault()).stream()
//                                .map(String::valueOf).collect(Collectors.toList()));//Tcu上报故障
//                    }
//                    //充电接口故障
...
//                                List<String> pcuReportFaultList = FaultParseUtil.parsePcuFault(moduleFault.getPcuReportFault())
//                                        .stream().map(f -> f + FileUtil.BAR + moduleFault.getModuleAddr()).collect(Collectors.toList());
////                                pcuReportFaultList.forEach(faultCode -> moduleFaultAddrMap.put(faultCode, moduleFault.getModuleAddr()));
//                                pileFaultCodes.addAll(pcuReportFaultList);
//                            }
//                        }
//                    }
//                    //对比电桩故障码 保存入库 后期调整
//                    Set<Integer> updateFaultCodes = Sets.newHashSet();
//
//                    //根据设备编号和多个故障码查询设备电桩事件数据
//                    DeviceService deviceService = SpringBeanUtil.getBean(DeviceService.class);
//                    Set<Integer> faultCodes = Stream.concat(pileFaultCodes.stream().map(pileFaultCode -> {
//                        if (pileFaultCode.contains(FileUtil.BAR)) {
//                            return Integer.parseInt(pileFaultCode.split(FileUtil.BAR)[0]);
//                        }
//                        return Integer.parseInt(pileFaultCode);
//                    }).collect(Collectors.toSet()).stream(), itfFaultCodeMap.values().stream().flatMap(Collection::stream).collect(Collectors.toSet()).stream())
...
//                            if (!lastPileFaultCodes.contains(pileFaultCode)) { //新产生的故障
//                                addAlarmRecordList.add(PileRecordUtil.addAlarmRecord(pilesCode, null, pileFaultCode, pileRealModel.getFeatureCode(), moduleFaultAddrMap.get(pileFaultCode), faultEventMap));
//                            }
//                        }
//                        for (Integer lastPilesFaultCode : lastPileFaultCodes) {
//                            if (!pileFaultCodes.contains(lastPilesFaultCode)) {//恢复的故障
//                                updateFaultCodes.add(lastPilesFaultCode);
//                            }
//                        }
//                    } else {
//                        for (Integer pileFaultCode : pileFaultCodes) {
...
//                                            addAlarmRecordList.add(PileRecordUtil.addAlarmRecord(pilesCode, gunCode.toString(), faultCode, pileRealModel.getFeatureCode(), moduleFaultAddrMap.get(faultCode), faultEventMap));
//                                        }
//                                    }
//                                    //恢复的枪故障
//                                    for (Integer faultCode : lastItfFaults) {
//                                        if (!newItfFaults.contains(faultCode)) {
//                                            updateFaultCodes.add(faultCode);
//                                        }
//                                    }
//                                } else {
//                                    //新产生的故障
...
//                            updateFaultCodes.addAll(lastItfFaults);
//                        }
//                    }
//
//                    if (CollectionUtils.isNotEmpty(updateFaultCodes)) {
//                        updateAlarmRecordMap.put(pilesCode, updateFaultCodes);
//                    }
//
...
//            if (MapUtils.isNotEmpty(updateAlarmRecordMap)) {
//                updateAlarmRecordMap.forEach((key, value) -> updateAlarmRecordList.addAll(alarmRecordDao.findAllByDeviceCodeAndFaultCodeInAndAlarmStatus(key, value, 0)));
//            }
