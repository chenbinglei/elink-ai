...
@Slf4j
public class SmWebControlHandler {
     ...
     *
     * @param pileStartVo 包含响应信息的模型对象，其中包括了通用消息模型和特定的枪支代码等信息。
     */
    public static void startCmdRes(PileStartResponseSubscribeVo pileStartVo) {
        // 根据响应信息和枪支代码等生成唯一的键值，用于在需求模型映射中查找对应的模型
        String keyId = pileStartVo.getPilesCode() + pileStartVo.getGunCode() + SMV2gConstant.STARTCMD;
        ...
        if (PlatformControlDemand.demandModelMap.containsKey(keyId)) {
            ...
            demandModel.setRecCode(pileStartVo.getResponseResult());
            demandModel.setRecMsg(pileStartVo.getFaileReason());
            demandModel.setSerialNum(pileStartVo.getRecordId());
            log.info("外网MQTT平台V2G协议启动响应需求:{}", demandModel);
        }
    }
     ...
     *
     * @param pileStopResponseVo 停止响应的参数模型，包含枪支代码、错误原因和详细错误信息等。
     */
    public static void stopCmdRes(PileStopResponseSubscribeVo pileStopResponseVo) {
        ...
        if (PlatformControlDemand.demandModelMap.containsKey(keyId)) {
            ...
            log.info("外网MQTT平台V2G协议停止响应需求:{}", demandModel);
        }
    }
     ...
     *
     * @param pileCtrlResponseSubscribeVo 功率控制响应的参数模型，包含枪支代码、错误原因和详细错误信息等。
     */
    public static void powerCtrlRes(PileCtrlResponseSubscribeVo pileCtrlResponseSubscribeVo) {
        // 根据消息模型中的通用消息部分获取堆栈代码，并结合枪支代码和停止命令的标识生成唯一键
        String keyId = pileCtrlResponseSubscribeVo.getPilesCode() + pileCtrlResponseSubscribeVo.getGunCode() + SMV2gConstant.POWERCTRL;
        ...
        if (PlatformControlDemand.demandModelMap.containsKey(keyId)) {
            ...
            PlatformDemandModel demandModel = PlatformControlDemand.demandModelMap.get(keyId);
            demandModel.setRecFlag(true);
            demandModel.setRecCode(pileCtrlResponseSubscribeVo.getResponseResult());
            log.info("外网MQTT平台V2G协议功率控制响应需求:{}", demandModel);
        }
    }
     ...
     * 如果是，则创建一个启动事件的视图对象并准备调用平台V2G协议提供的接口（当前TODO状态）。
     *
     * @param pileStartEventVo 包含枪机信息和启动事件数据的模型对象。
     */
    public static void pushStartEvent(PileStartEventSubscribeVo pileStartEventVo) {
        try {
            // 获取堆垛机代码和枪机代码
            String pileCode = pileStartEventVo.getPilesCode();
            String gunCode = String.valueOf(pileStartEventVo.getGunCode());
            ...
            if (pileRealModel != null) {
                ...
                if (pileRealModel.getGunRealModelMap().containsKey(gunCode)) {
                    ...
                    if (StringUtil.isNotEmpty(gunRealModel.getStarter()) && Objects.equals(gunRealModel.getStarter(), PlatformUtil.starter)) {
                        ...
                        result.setGunCode(pileStartEventVo.getGunCode());
                        result.setFailReason(pileStartEventVo.getEventResult());
                        result.setFailDetailReason(pileStartEventVo.getFaileReason());
                        result.setStopDetail(pileStartEventVo.getStopDetail());
                        result.setStartTime(SunMaxUtil.timeStamp8Date(pileStartEventVo.getStartTime()));
                        result.setSerialNum(pileStartEventVo.getRecordId());

                        // 调用平台V2G协议提供的接口，发送启动事件信息
                        log.info("外网MQTT推送平台V2G协议启动事件:{}", result);
                        ...
                        if (platformData != null) {
                            ...
                            log.info("外网MQTT推送平台V2G协议启动事件响应结果:{}", PlatformUtil.post(platformData.getUrl(), "pushStartEvent", requestVo));
                        } else {
                            log.error("外网MQTT推送平台V2G协议启动事件失败，未找到对应平台ID");
                        }
                    }
                }
            }
     ...
     * 如果是，则创建一个电桩状态的视图对象并准备调用平台V2G协议提供的接口（当前TODO状态）。
     *
     * @param pileStateMap 包含电桩信息和电桩状态数据的模型对象。
     *                     该对象应包含电桩的运行状态和接口状态等详细信息。
     */
    public static void pushPileStatus(Map<String, PileStateSubscribeVo> pileStateMap) {

        try {
            List<Integer> gunWorkStates = Arrays.asList(1, 2, 4, 5);
            pileStateMap.forEach((pileCode, value) -> {
                PileRealModel pileRealModel = RedisGeneralUtil.getPileRealModel(pileCode);
                if (PlatformUtil.pileRunMap.containsKey(pileCode) && pileRealModel != null) {
                    ...
                    pileStatus.setWorkStatus(value.getWorkState());

                    // 遍历接口状态，构建电桩状态视图的接口数据列表
                    final boolean[] isRun = {false};
                    List<PlatformPileStatusVo.ItfData> itfDataList = value.getGun_state().stream().map(itfStatus -> {
                        PlatformPileStatusVo.ItfData result = new PlatformPileStatusVo.ItfData();
                        // 检查并设置枪的工作状态
                        if (!isRun[0]) {
                            if (gunWorkStates.contains(itfStatus.getWorkState())) {
                                isRun[0] = true;
                            }
                        }
                        // 填充接口数据
                        result.setGunCode(itfStatus.getGunCode());
                        result.setGunWorkStatus(itfStatus.getWorkState());
                        result.setParkingStatus(0);
                        result.setVehicleConnStatus(itfStatus.getVehicleConnState());
                        // 如果是运行状态且有对应枪的状态数据，填充详细运行数据
                        String gunCode = String.valueOf(itfStatus.getGunCode());
                        if (gunWorkStates.contains(itfStatus.getWorkState()) && pileRealModel.getGunRealModelMap().containsKey(gunCode)) {
                            PileRealModel.GunRealModel gunRealModel = pileRealModel.getGunRealModelMap().get(gunCode);
                            if (StringUtil.isNotEmpty(gunRealModel.getStarter()) && Objects.equals(gunRealModel.getStarter(), PlatformUtil.starter)) {
                    ...
                    if (isRun[0]) {
                        ...
                        log.info("外网MQTT推送平台V2G协议电桩状态及数据:{}", pileStatus);
                        PlatformDataDto platformData = ProtocolRunner.getPlatformData(pileCode);
                        if (platformData != null) {
                            PlatformRequestVo requestVo = new PlatformRequestVo();
                            requestVo.setPlatformId(platformData.getPlatformId());
                            requestVo.setData(PlatformUtil.encryptData(platformData, JSON.toJSONString(pileStatus)));
                            log.info("外网MQTT推送平台V2G协议电桩状态及数据响应结果:{}", PlatformUtil.post(platformData.getUrl(), "pushPileStatus", requestVo));
                        } else {
                            log.error("外网MQTT推送平台V2G协议电桩状态及数据失败，未找到对应平台ID");
                        }
                    } else {
                        PlatformUtil.pileRunMap.remove(pileCode);
                        log.info("外网MQTT平台V2G协议从运行映射中移除电桩:{}", pileCode);
                    }
                }
            });
     ...
     * 如果是，则创建一个电桩停止事件的视图对象并准备调用平台V2G协议提供的接口（当前TODO状态）。
     *
     * @param pileStopEventVo 包含电桩信息和电桩停止事件数据的模型对象。
     *                        其中包含了电桩编码、枪编码、停止详细原因、结束时间戳和序列号等信息。
     */
    public static void pushStopEvent(PileStopEventSubscribeVo pileStopEventVo) {
        try {
            // 从消息模型中提取充电桩编码和枪编码
            String pileCode = pileStopEventVo.getPilesCode();
            String gunCode = String.valueOf(pileStopEventVo.getGunCode());
            ...
            if (pileRealModel != null) {
                ...
                if (pileRealModel.getGunRealModelMap().containsKey(gunCode)) {
                    ...
                    if (gunRealModel != null && StringUtil.isNotEmpty(gunRealModel.getStarter()) && Objects.equals(gunRealModel.getStarter(), PlatformUtil.starter)) {
                        ...
                        result.setPileCode(pileCode);
                        result.setGunCode(pileStopEventVo.getGunCode());
                        result.setFailDetailReason(pileStopEventVo.getFaileReason());
                        result.setEndTime(SunMaxUtil.timeStamp8Date(pileStopEventVo.getStopTime()));
                        result.setSerialNum(pileStopEventVo.getRecordId());

                        // 调用平台V2G协议提供的接口，推送电桩停止事件信息
                        // 这里需要实现调用外部接口的逻辑，将stopEventVo中的信息推送出去
                        log.info("外网MQTT推送平台V2G协议停止事件:{}", result);
                        PlatformDataDto platformData = ProtocolRunner.getPlatformData(pileCode);
                        if (platformData != null) {
                            PlatformRequestVo requestVo = new PlatformRequestVo();
                            requestVo.setPlatformId(platformData.getPlatformId());
                            requestVo.setData(PlatformUtil.encryptData(platformData, JSON.toJSONString(result)));
                            log.info("外网MQTT推送平台V2G协议停止事件响应结果:{}", PlatformUtil.post(platformData.getUrl(), "pushStopEvent", requestVo));
                        } else {
                            log.error("外网MQTT推送平台V2G协议停止事件失败，未找到对应平台ID");
                        }
     ...
     *                     4. 准备将此记录信息推送给平台V2G协议平台。目前推送逻辑尚未实现，TODO注释标明了后续需要完成的任务。
     *                     5. 如果在处理过程中发生异常，将记录错误日志。
     */
    public static void pushPileRecord(PileRecordSubscribeVo pileRecordVo) {
        try {
            // 提取充电桩编码和枪编码
            ...
            if (pileRealModel != null && pileRealModel.getGunRealModelMap().containsKey(gunCode)) {
                ...
                if (gunRealModel != null && StringUtil.isNotEmpty(gunRealModel.getStarter()) && Objects.equals(gunRealModel.getStarter(), PlatformUtil.starter)) {
                    ...
                    stopEvent.setEndTime(SunMaxUtil.timeStamp8Date(pileRecordVo.getEndTime()));
                    stopEvent.setSerialNum(pileRecordVo.getRecordId());

                    // 调用平台V2G协议提供的接口，推送电桩停止事件信息
                    // 这里需要实现调用外部接口的逻辑，将stopEventVo中的信息推送出去
                    log.info("外网MQTT推送平台V2G协议电桩补偿停止事件:{}", stopEvent);
                    PlatformDataDto platformData = ProtocolRunner.getPlatformData(pileCode);
                    if (platformData != null) {
                        PlatformRequestVo requestVo = new PlatformRequestVo();
                        requestVo.setPlatformId(platformData.getPlatformId());
                        requestVo.setData(PlatformUtil.encryptData(platformData, JSON.toJSONString(stopEvent)));
                        log.info("外网MQTT推送平台V2G协议电桩补偿停止事件响应结果:{}", PlatformUtil.post(platformData.getUrl(), "pushStopEvent", requestVo));
                    } else {
                        log.error("外网MQTT推送平台V2G协议电桩补偿停止事件失败，未找到对应平台ID");
                    }
                    ...
                    result.setStartTime(SunMaxUtil.timeStamp8Date(pileRecordVo.getStartTime()));
                    result.setEndTime(SunMaxUtil.timeStamp8Date(pileRecordVo.getEndTime()));
                    ...
                    if (StringUtil.isNotEmpty(pileRecordVo.getRateId())) {
                        result.setRateTemplateId(SunMaxUtil.toHexString(pileRecordVo.getRateId()));
                    }
                    ...
                    log.info("外网MQTT推送平台V2G协议推送电桩记录:{}", result);
                    if (platformData != null) {
                        PlatformRequestVo requestVo = new PlatformRequestVo();
                        requestVo.setPlatformId(platformData.getPlatformId());
                        requestVo.setData(PlatformUtil.encryptData(platformData, JSON.toJSONString(result)));
                        log.info("外网MQTT推送平台V2G协议电桩记录响应结果:{}", PlatformUtil.post(platformData.getUrl(), "pushPileRecord", requestVo));
                    } else {
                        log.error("外网MQTT推送平台V2G协议电桩记录失败，未找到对应平台ID");
                    }
                }
            }
        } catch (Exception e) {
