        ...
        if (body != null && StringUtil.isNotEmpty(body.getDevId()) && StringUtil.isNotEmpty(body.getCmd())) {
            ...
            switch (body.getCmd()) {
                case CmdConstant.CMD_RATE_REQ://费率请求
                    InterMqttDataHandler.rateReq(deviceId, JSONObject.toJavaObject(data, RateReqVo.class));
                    break;
//                case CmdConstant.CMD_RATE_RES://费率响应
//                    break;
                case CmdConstant.CMD_SUBLINKUP_NOTIFY://子设备上线通知
                    InterMqttDataHandler.subLinkUpNotify(deviceId, JSONObject.toJavaObject(data, SubLinkUpNotifyVo.class));
                    break;
                case CmdConstant.CMD_CBINFO_NOTIFY://硬件控制板软硬件信息通知
                    InterMqttDataHandler.cbInfoNotify(deviceId, JSONObject.toJavaObject(data, CbInfoNotifyVo.class));
                    break;
//                case CmdConstant.CMD_START://启动命令
//                    break;
                case CmdConstant.CMD_START_RES://启动响应
                    StartResVo startResVo = JSONObject.toJavaObject(data, StartResVo.class);
                ...
                case CmdConstant.CMD_START_EVENT://启动事件
                    StartEventVo startEventVo = JSONObject.toJavaObject(data, StartEventVo.class);
...
//                    break;
                case CmdConstant.CMD_STOP_RES://停止响应
                    StopResVo stopResVo = JSONObject.toJavaObject(data, StopResVo.class);
                ...
                case CmdConstant.CMD_STOP_EVENT://停止事件
                    StopEventVo stopEventVo = JSONObject.toJavaObject(data, StopEventVo.class);
...
//                    break;
                case CmdConstant.CMD_POWERCONTROL_RES://功率控制响应
                    PowerControlResVo powerControlResVo = JSONObject.toJavaObject(data, PowerControlResVo.class);
                ...
                case CmdConstant.CMD_PILE_DATA_REPORT://电桩数据上报
                    InterMqttDataHandler.pileDataReport(deviceId, JSONObject.toJavaObject(data, PileDataReportVo.class));
                    break;
                case CmdConstant.CMD_PILE_FAULT_REPORT://电桩故障上报
                    InterMqttDataHandler.pileFaultReport(deviceId, JSONObject.toJavaObject(data, PileFaultReportVo.class));
                    break;
                case CmdConstant.CMD_PILE_STATUS_REPORT://电桩状态上报
                    PileStatusReportVo pileStatusReportVo = JSONObject.toJavaObject(data, PileStatusReportVo.class);
                ...
                case CmdConstant.CMD_PILE_RECORD_REPORT://充电记录上报
                    PileRecordReportVo pileRecordVo = JSONObject.toJavaObject(data, PileRecordReportVo.class);
...
//                    break;
                case CmdConstant.CMD_AuthenticationRequest://鉴权请求
                    InterMqttDataHandler.authenticationReq(deviceId, JSONObject.toJavaObject(data, AuthenticationReqVo.class));
                    break;
//                case CmdConstant.CMD_AuthenticationResponse://鉴权响应
//                    break;
                case CmdConstant.CMD_StrategySettingRequest://策略设置
                    InterMqttDataHandler.strategySettingReq(deviceId, JSONObject.toJavaObject(data, StrategySettingReqVo.class));
                    break;
...
//                    break;
                case CmdConstant.CMD_DevDataBlockRequest://设备升级-数据块请求
                    InterMqttDataHandler.devDataBlockReq(deviceId, JSONObject.toJavaObject(data, DevDataBlockReqVo.class));
                    break;
//                case CmdConstant.CMD_DevDataBlockResponse://设备升级-数据块响应
//                    break;
                case CmdConstant.CMD_DevUpdateReport://设备升级-升级结果上报
                    InterMqttDataHandler.devUpdateReport(deviceId, JSONObject.toJavaObject(data, DevUpdateReportVo.class));
                    break;
//                case CmdConstant.CMD_RATE_SET://费率下发
//                    break;
                case CmdConstant.CMD_RATE_SET_RES://费率下发响应
                    InterMqttDataHandler.rateSetRes(deviceId, JSONObject.toJavaObject(data, RateSetResVo.class));
                    break;
...
//                    break;
                case CmdConstant.CMD_PileDataResponse://电桩运行数据请求响应
                    InterMqttDataHandler.pileDataRes(deviceId, JSONObject.toJavaObject(data, PileDataResVo.class));
                    break;
//                case CmdConstant.CMD_VehicleInfoRequest://车辆信息请求
//                    break;
                case CmdConstant.CMD_VehicleInfoResponse://车辆信息请求响应
                    InterMqttDataHandler.vehicleInfoRes(deviceId, JSONObject.toJavaObject(data, VehicleInfoResVo.class));
                    break;
//                case CmdConstant.CMD_PileRecordReportRequest://电桩记录查询
//                    break;
                case CmdConstant.CMD_PileRecordReportResponse://电桩记录查询命令响应
                    InterMqttDataHandler.pileRecordReportRes(deviceId, JSONObject.toJavaObject(data, PileRecordReportResVo.class));
                    break;
                case CmdConstant.CMD_PileRecordReportInfoResponse://电桩记录查询结果响应
                    InterMqttDataHandler.pileRecordReportInfoRes(deviceId, JSONObject.toJavaObject(data, PileRecordReportInfoResVo.class));
                    break;
...
//                    break;
                case CmdConstant.CMD_PILE_BMSINFO_REPORT://充电bms信息
                    InterMqttDataHandler.pileBmsInfoReport(deviceId, JSONObject.toJavaObject(data, PileBmsInfoReportVo.class));
                    break;
                case CmdConstant.CMD_MODBUS_INFO_REPORT://modbus数据
                    InterMqttDataHandler.modbusInfoReport(deviceId, JSONObject.toJavaObject(data, ModbusInfoReportVo.class));
                    break;
                case CmdConstant.CMD_Dev_HeartBeat://设备心跳
                    InterMqttDataHandler.pileHeartBeat(JSONObject.toJavaObject(data, DevHeartBeatVo.class));
                    break;
                case CmdConstant.CMD_PILE_LOGREPORT://日志数据上报
                    InterMqttDataHandler.pileLogReport(deviceId, JSONObject.toJavaObject(data, PileLogReportVo.class));
                    break;
                case CmdConstant.CMD_PILE_RESET_RESULT://电桩复位响应
                    InterMqttDataHandler.pileResetResult(deviceId, JSONObject.toJavaObject(data, PileResetResultVo.class));
                case CmdConstant.CMD_PILE_SETQR_RES://设置二维码前缀响应
                    InterMqttDataHandler.pileSetQrRes(JSONObject.toJavaObject(data, PileSetQrResVo.class));
                    break;
            }
        }
    }
