...
public class SeriesInfoController {
    ...
    @Operation(summary = "批量删除组件库信息")
    @WebLog("组串管理-批量删除组件库信息")
    
    @Parameters({
            @Parameter(name = "ids", description = "多个组件id(['aaa','bbb'])"),
    })
    ...
    @Operation(summary = "查询组件型号列表")
    
    @Parameters({
            @Parameter(name = "moduleFactory", description = "组件厂家")
    })
    ...
    @Operation(summary = "保存组件配置列表")
    @WebLog("组串管理-保存组件配置列表")
    
    @Parameters({
            @Parameter(name = "deviceId", description = "所属设备id"),
            @Parameter(name = "seriesConfigChangeVos", description = "组串配置信息json字符串对象列表")
    })
    ...
    @Operation(summary = "根据设备id查询组串配置信息")
    ...
    @Operation(summary = "根据多个设备id清除组串配置列表")
    @WebLog("组串管理-清除组串配置列表")
    
    @Parameters({
            @Parameter(name = "deviceIds", description = "多个所属设备id(['aaa','bbb'])")
    })
    ...
    @Operation(summary = "根据站点id查询逆变器设备列表")
    
    @Parameters({
            @Parameter(name = "siteId", description = "所属站点id"),
            @Parameter(name = "configStatus", description = "配置状态 1-未配置 2-已配置"),
    ...
    @Operation(summary = "根据站点id查询逆变器设备型号列表")
