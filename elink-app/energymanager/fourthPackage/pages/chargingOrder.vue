  ...
  <view class="containers">
    <view class="card-title">
      ...
      <view class="filter-card">
        <selectIf :type="'chargingOrder'" @handleTimeRange="handleTimeRange" @handleOrderStatus="handleOrderStatus"
        ...
        <view class="content-list-item" v-for="(item, index) in siteListCard" :key="index">
          <view class="site-title">
            ...
            <view :class="['site-status', 'bgc-' + item.orderStatus]">
              {{ $filters.chargingState(item.orderStatus) }}
            </view>
          </view>
          ...
          <view class="charge-power-money">
            <view class="charge-power-money-item">
              ...
              <view class="charge-power-money-item-label">充电电量（kW ）</view>
            </view>
            <view class="charge-power-money-item">
          ...
          <view class="card-button">
            ...
            <!-- <view class="button-item btn2" v-if="item.orderStatus == 1 || item.orderStatus == 6"
              @click="stopCharge(item)">

              <view class="button-text">结束充电</view>
            </view> -->
            <view class="button-item btn1" @click="applyRefund(item, 2)" v-if="item.prepayMoney > 0"
              :class="{ disabled: item.orderStatus == 1 || item.orderStatus == 6 }">
              <view class="button-text">补单/退款</view>
            </view>

            <view class="button-item btn1" @click="applyRefund(item, 3)" v-else
              :class="{ disabled: item.orderStatus == 1 || item.orderStatus == 6 }">
              <view class="button-text">补单/退款</view>
            </view>
...
const pageId = ref('chargingOrder')
...
const goOrderDetail = (item) => {
  uni.navigateTo({
    url: `/fourthPackage/pages/OrderDetail?orderId=${item.id}`
  });
}
...
// 申请退款
// 申请退款 / 补单（完全对齐你提供的Vue逻辑，不新增接口）
const applyRefund = (item, operateType) => {
  // ==============================================
  // 类型 2 → 有预付金额 → 打开退款弹窗（Vue 原版逻辑）
  // ==============================================
  if (operateType == 2) {
    alertDialog.value.open('bottom')
    refoundItem.value = item
    return
  }

  // ==============================================
  // 类型 3 → 免支付订单 / 挂起订单 → 调用 updateOrderStatus
  // 完全对齐你Vue里面的逻辑，不新增任何接口
  // ==============================================
  if (operateType == 3) {
    // 完全沿用你 Vue 里的提示语
...
const cancel = () => {
  ...
  applyRefundModel.value = { refundMoney: '', refundReason: '' }
}
