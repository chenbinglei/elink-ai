...
export function ajax (options) {
	console.log("请求接口：" + options.nologinRequired, uni.getStorageSync('BASE_URL'));
	return new Promise((resolve, reject) => {
		...
		// ===================== 只加这一段 =====================
		// 强制把 orderTypes 转成数组
		if (options.data && options.data.orderTypes) {
			options.data.orderTypes = JSON.stringify(options.data.orderTypes);
		}
		// =====================================================
...
export function dj_upload (options) {
	return new Promise((resolve, reject) => {
		...
		console.log(options);
		// #ifdef MP-WEIXIN
		options.name = 'annexFile';
		options.filePath = options.files.uri;
		// #endif
