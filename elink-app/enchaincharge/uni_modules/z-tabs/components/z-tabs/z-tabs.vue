			...
			<scroll-view ref="z-tabs-scroll-view" class="z-tabs-scroll-view" :scroll-x="true" :scroll-left="scrollLeft" :show-scrollbar="false" :scroll-with-animation="isFirstLoaded" @scroll="scroll">
				<view class="z-tabs-list-container" :style="[tabsListStyle]">
					<view class="z-tabs-list" :style="[tabsListStyle, {marginTop: -finalBottomSpace+'px'}]">
						...
						<view :ref="`z-tabs-item-${index}`" :id="`z-tabs-item-${index}`" class="z-tabs-item" :style="[tabStyle,item.disabled?disabledStyle:(currentIndex===index?activeStyle:inactiveStyle)]" v-for="(item,index) in list" :key="index" @click="tabsClick(index,item)">
							<view class="z-tabs-item-title-container">
								<text :class="{'z-tabs-item-title':true,'z-tabs-item-title-disabled':item.disabled}" 
									:style="[{color:item.disabled?disabledColor:(currentIndex===index?activeColor:inactiveColor)}]">
									{{item[nameKey]||item}}
								</text>
...
<script>
	...
	export default {
		...
		props: {
			...
			activeColor: {
				type: String,
				default: _gc('activeColor','#15E8AF')
			},
			//未激活状态tab的颜色
			inactiveColor: {
				type: String,
				default: _gc('inactiveColor','#979797')
			},
			//禁用状态tab的颜色
			disabledColor: {
				type: String,
