...
| v4.0 | 2026-06-12 | AI | P4-BC-hotfix 三前端 router/index.js 加载报错修复：`node --check` 静态证据确认 derms/linkos/tycvs 三 router 工作树语法错误，HEAD 版本可解析 → `git checkout HEAD --` 还原；linkos/tycvs `getComponent` 改造为基于 `import.meta.glob("@/views/**/*.vue")` 的查表方案以适配 Vite 多级动态 import 限制；tycvs `2DVisualization/.../gallery.js` 4 处 `require()` 替换为 ESM `import` 静态资源；删除残留 `index.js.bak`；3 dev server（9000/9001/9002）入口 + main.js + router 全部 200，linkos/tycvs 预览无错误，derms 仅外部 mapbox.com 网络错误（与本次修复无关） |
