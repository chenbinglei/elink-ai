    ...
    <div v-if="isShowHeader" class="title">
      ...
      <div class="rightTopButIcon">
        <template v-if="isCollapseBut">
          <span class="iconfont pointer"
            :class="[defaultExpandAll ? 'icon-zuocelan-lanzhankai' : 'icon-zuocelan-lansuohui']"
            @click="clickRightBut(1)"></span>
        </template>
    ...
    <div class="bottomContent">
      ...
      <div class="list_menu">
        <div class="list_menu_content scrollbarStyle">
          <el-tree ref="treeBoxRef" :class="customTreeClass" :current-node-key="currentLivingId" :data="list"
            :default-checked-keys="defaultCheckedKeys" :default-expand-all="defaultExpandAll"
            :default-expanded-keys="defaultExpandedKeys" :filter-node-method="filterTreeNodeFun"
            :highlight-current="!show_checkbox" :indent="12" :props="treeProps" :show-checkbox="show_checkbox"
            auto-expand-parent :expand-on-click-node="expandOnClickNode" check-on-click-node class="tree-line"
            :node-key="nodeKey" @check-change="checkChangeFun" @node-click="clickTreeNode">
            <template #default="{ node, data }">
              <div class="custom-tree-node">
                ...
                <div style="position: absolute;right: 10px;top: 11px;" v-if="moreOperate">
                  <el-dropdown @command="(command, $event) => clickMoreOperate(command, $event, data)">
                    <span class="el-dropdown-link" @click.stop><img src="@/assets/image/more-icon.png" alt="" width="24"
                        height="24"></span>
                    <template #dropdown>
                      <el-dropdown-menu>
                        <el-dropdown-item command="addOrgan">新增组织</el-dropdown-item>
                        <el-dropdown-item command="editOrgan">编辑组织</el-dropdown-item>
                        <el-dropdown-item command="deleteOrgan" :disabled="!data.parentId">删除组织</el-dropdown-item>
                      </el-dropdown-menu>
                    </template>
                  </el-dropdown>
                </div>
  ...
  setup (props) {
    ...
    const getChildrenFirstFun = (item) => {
      if (item.children && item.children.length) {
        ...
        that.defaultExpandedKeys.push(item[props.nodeKey]);
      } else {
        nextTick(() => {
          treeBoxRef.value.setCurrentKey(item[props.nodeKey]);
          // clickTreeNode(item);
        });
      }
    };
    ...
    const clickTreeNode = (item) => {
      ...
      that.currentLivingId = item[props.nodeKey];
    ...
    // 点击更多操作
    const clickMoreOperate = (type, event, data) => {
      emit("more", { type: type, data });
    };
    ...
    const setTreeSelectCheck = (isSelectAll) => {
      ...
      if (isSelectAll) {
        ...
        for (let i = 0; i < allCheckArray.length; i++) defaultCheckedKeys.push(allCheckArray[i][props.nodeKey]);
    ...
    // 过滤树形控件数据
    const filterTreeNodeFun = (value, data, node) => {
      if (!value) return true;
      if (value === "body" && props.treeProps.children === "body") {
        // console.log(node);
        return !(data.type !== "jsonArray" && (!data.body || !data.body.length));
      }
    ...
    return {
      ...toRefs(that), listLengthFun, getChildrenFirstFun, clickTreeNode, treeBoxRef, watchArray, setTreeSelectCheck, searchInput, clickRightBut,
      clickBatchButton, cancelSelect, confirmSelect, setCheckedKeysFun, checkChangeFun, filterTreeNodeFun, watchId, watchHandleMenuTopIndex, clickClosedIcon, clickMoreOperate
    };
  }
});
</script>
...
.leftArrowClass {
  ...
  .el-tree-node {
    ...
    &::before {
      ...
      top: -22px;
      border-width: 1px;
      border-left: 1px solid rgba(189, 202, 212, 0.2);
    }
    ...
    &::after {
      ...
      top: 23px;
      border-width: 1px;
      border-top: 1px solid rgba(189, 202, 212, .2);
    }
  }
  ...
  .is-current {
    --el-color-primary-light-9: rgba(0, 88, 137, 0.3);
