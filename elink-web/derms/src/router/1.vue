...
function findFirstVisibleWithChildren (data) {
  ...
  //   if (item.hidden === 0 && item.children) {
  //     return item;
  //   }
  // }
  // return null; // 如果没有符合条件的项
  for (let item of data) {
...
router.beforeEach((to, from, next) => {
  console.log('404页面，跳转回首页', to, from, findFirstVisibleWithChildren(new_sidebar_list))

  if (to.path === '/404' && from.path === '/404') {
    return next(findFirstVisibleWithChildren(new_sidebar_list))
    //  if (findFirstVisibleWithChildren(new_sidebar_list)?.children?.length === 0) {
    //   return next(findFirstVisibleWithChildren(new_sidebar_list).path)
    // } else {
    //   return next(findFirstVisibleWithChildren(new_sidebar_list).children[0].path);
    // }
  }
  // 未登录时跳转到登录页
  ...
  if ((to.path === '/login'||to.path === '/404') && from.path === '/') {
    // console.log('404页面，跳转回首页',findFirstVisibleWithChildren(new_sidebar_list))
    return next(findFirstVisibleWithChildren(new_sidebar_list))
    // if (findFirstVisibleWithChildren(new_sidebar_list)?.children?.length === 0) {
    //   return next(findFirstVisibleWithChildren(new_sidebar_list).path)
    // } else {
