<template>
  <div>
    ...
    <div class="main-content" v-loading="loading" ref="tableContentRef" v-resize="setTableMaxHeight">
      <el-table :data="tableData" ref="tableRef" style="table-layout: fixed" border stripe :max-height="tableMaxHeight">
        <el-table-column prop="systemName" label="系统名称" min-width="120" />
        <el-table-column label="场站名称" width="220">
          <template #default="scope">
            <span style="color: #00ccffff">{{ scope.row.siteName }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="location" label="所属省市" min-width="120" />
        <el-table-column prop="capacity" label="装机容量" min-width="120"><template #default="scope">
            {{ scope.row.capacity }}kW
          </template></el-table-column>
        <el-table-column label="功率曲线" width="200">
          <template #default="scope">
        ...
        <el-table-column label="今日充电量" prop="chargeOrderQt" min-width="120">
          <template #default="scope">
            <!-- {{ scope.row.chargeOrderQt }}kW -->
            {{
              scope.row.chargeOrderQt ? `${scope.row.chargeOrderQt}kWh` : "--"
            }}
          </template></el-table-column>
        <el-table-column label="今日V2G电量" prop="dischargeOrderQt" min-width="120">
          <template #default="scope">
            <!-- {{ scope.row.dischargeOrderQt }}kW -->
            {{
              scope.row.dischargeOrderQt
                ? `${scope.row.dischargeOrderQt}kWh`
                : "--"
            }}
          </template></el-table-column>
        <el-table-column prop="chargeOrderNum" label="今日充电次数" min-width="120">
          <template #default="scope">
            {{ scope.row.chargeOrderNum==null ? "--" : scope.row.chargeOrderNum }}
          </template>
        </el-table-column>
        <el-table-column prop="avgChargeQt" label="今日枪均充电量" min-width="120">
          <template #default="scope">
            <!-- {{ scope.row.avgChargeQt }}kW -->
            {{ scope.row.avgChargeQt ? `${scope.row.avgChargeQt}kWh` : "--" }}
          </template></el-table-column>
        <el-table-column prop="chargeSuccessRatio" label="一次充电成功率" min-width="160">
          <template #default="scope">
            <!-- {{
              scope.row.chargeSuccessRatio
                ? scope.row.chargeSuccessRatio + "%"
                : ""
            }} -->
            {{
      ...
      <div v-if="showRightPanel" class="right-panel">
        ...
        <div class="chart-area">
          <ECharts v-if="selectedRowData && selectedRowData.historys" :key="selectedRowData.siteId" :data="selectedRowData.historys" width="100%"
            :height="250" />
        </div>
        <!-- 状态模块 -->
        <div class="status-module-container">
          <StatusModule v-if="selectedRowData && selectedRowData.devices" :normal-count="selectedRowData.devices[0]?.normal ?? 0"
            :unregistered-count="selectedRowData.devices[0]?.unregistered ?? 0" :error-count="selectedRowData.devices[0]?.error ?? 0"
      ...
      <!-- 分页 -->
      <div class="pagination-wrapper" ref="tablePaginationRef">
  ...
  setup (props, { emit }) {
    ...
    const queryPage = async () => {
      try {
        ...
        const res = await batterySupplyPage({
          page,
          size,
          area,
          word,
        ...
        emit("isVisible", res.data.totalSize ? true : false, 3);
        that.loading = false;
        drawCharts(); // 数据更新后重绘
      } catch (error) {
        that.loading = false;
