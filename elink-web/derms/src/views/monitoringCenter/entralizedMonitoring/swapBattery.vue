<template>
  ...
  <div class="main-content" v-loading="loading" ref="tableContentRef" v-resize="setTableMaxHeight">
    <el-table :data="tableData" ref="tableRef" style="table-layout: fixed; width: 100%" stripe :max-height="tableMaxHeight">
      <el-table-column label="场站名称" min-width="200">
        <template #default="scope">
          <span style="color: #00ccffff">{{ scope.row.systemName }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="location" label="所属省市" min-width="150" />
      <el-table-column prop="chargePosition" label="充电仓位数" width="120"></el-table-column>
      <el-table-column label="功率曲线" min-width="180">
        <template #default="scope">
          <div v-if="scope.row.powerList">
      ...
      <el-table-column label="今日充电量" prop="dayChargeQt" min-width="150" />
      <el-table-column label="充电能耗占比" prop="chargeEnergyRatio" width="150" />
      <el-table-column prop="dayChangeNum" label="今日换电次数" min-width="150" />
      <el-table-column prop="soc" label="soc" min-width="150" />
    ...
    <div v-if="showRightPanel" class="right-panel">
      ...
      <div class="chart-area">
        <ECharts v-if="selectedRowData && selectedRowData.historys" :key="selectedRowData.siteId" :data="selectedRowData.historys" width="100%" :height="250" />
      </div>
      <!-- 状态模块 -->
      <div class="status-module-container">
        <StatusModule v-if="selectedRowData && selectedRowData.powerList" :normal-count="selectedRowData.powerList[0]?.normal ?? 0"
          :unregistered-count="selectedRowData.powerList[0]?.unregistered ?? 0" :error-count="selectedRowData.powerList[0]?.error ?? 0"
          :offline-count="selectedRowData.powerList[0]?.offline ?? 0" />
      </div>
...
export default defineComponent({
  ...
  setup (props, { emit }) {
    ...
    const calcWidth = (baseWidth) => {
      return baseWidth + (showRightPanel.value ? 0 : 35);
    };
    const tableData = ref([]);
    ...
    const togglePanel = (show, rowData = null) => {
      ...
      if (rowData) {
        ...
        const mergedHistorys =
          rowData.powerList?.flatMap((device) => device.historys || []) || [];
    ...
    const drawCharts = () => {
      setTimeout(() => {
        tableData.value.forEach((row, index) => {
          ...
          if (!canvas || !row.powerList || row.powerList.length === 0) return;
    ...
    const initFirstRow = () => {
      if (tableData.value.length > 0) {
        ...
        const mergedHistorys =
          firstRow.powerList?.flatMap((device) => device.historys || []) || [];
    ...
    const queryPage = async () => {
      ...
      const res = await batteryChangePage({
        page,
        size,
        area,
        word,
      ...
      emit("isVisible", res.data.totalSize ? true : false, 4);
      that.loading = false;
      drawCharts(); // 数据更新后重绘
    };
