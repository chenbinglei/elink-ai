<template>
  ...
  <div class="main-content" v-loading="loading" ref="tableContentRef" v-resize="setTableMaxHeight">
    <el-table :data="tableData" ref="tableRef" style="table-layout: fixed; width: 100%" stripe :max-height="tableMaxHeight">
      ...
      <el-table-column prop="pvType" label="光伏类型" min-width="120">
        <template #default="scope">
          {{ scope.row.pvType ? scope.row.pvType : "--" }}
        </template>
      </el-table-column>
      <el-table-column prop="consumMode" label="消纳方式" min-width="120">
        <template #default="scope">
          {{ scope.row.consumMode ? scope.row.consumMode : "--" }}
        </template>
      </el-table-column>
      <el-table-column prop="pvcapacity" label="装机容量" min-width="120">
        <template #default="scope">
          {{ scope.row.pvcapacity ? `${scope.row.pvcapacity}kWp` : "--" }}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="powerNormalize" label="实时功率归一化" width="150">
        <template #default="scope">
          {{ scope.row.powerNormalize ? scope.row.powerNormalize : "--"
          }}<span v-if="scope.row.powerNormalize">%</span>
        </template>
      </el-table-column>
      <el-table-column label="今日发电" min-width="180">
        <template #default="scope">
          {{
            scope.row.dailyPowerGeneration
              ? `${scope.row.dailyPowerGeneration}kWh`
              : "--"
          }}
          /
          {{
            scope.row.dailyPowerDuration
              ? `${scope.row.dailyPowerDuration}h`
              : "--"
          }}
        </template>
      </el-table-column>
...
export default defineComponent({
  ...
  setup (props, { emit }) {
    ...
    //   const res = await photovoltaicPage({
    //     page,
    //     size,
    //     area,
    //     word,
    ...
    const queryPage = async () => {
      try {
        ...
        emit("isVisible", res.data.totalSize ? true : false, 1);
