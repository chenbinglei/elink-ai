<template>
  <title-icon-view isContentHeight title="光伏发电量分析">
    <template #icon>
...
export default defineComponent({
  ...
  setup() {
    ...
    const queryPvGenerationCurveData = () => {
      let data = {};
      ...
      findPvSiteQtCurve({...data, siteIds: siteAllIds.value, timer: new Date()}).then(res => {
    ...
    const watchUpdateTimeNum = watch(() => updateTimeNum, () => {
      queryPvGenerationCurveData(); // 查询光伏发电量分析曲线数据
    }, {deep: true});

    return {...toRefs(that), queryPvGenerationCurveData, watchUpdateTimeNum, chartComponentRef, siteAllIds};
  }
});
</script>
