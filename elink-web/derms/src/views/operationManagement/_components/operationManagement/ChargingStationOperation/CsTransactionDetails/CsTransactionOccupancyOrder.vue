<template>
  <div class="app-container-right">
        ...
        <el-row :gutter="16">
          ...
          <el-col :md="8" :sm="12" :xl="6" :xs="24">
            <el-form-item label="商户平台：">
              <el-select v-model="formInline.tradeWay" clearable placeholder="全部" @change="queryTenantAccountList">
                <el-option v-for="item in payPlatformArray" :key="item.id" :label="item.name" :value="item.id"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :md="8" :sm="12" :xl="6" :xs="24">
            <el-form-item label="企业商户：">
              <el-select v-model="formInline.accountId" clearable placeholder="全部" :disabled="!formInline.tradeWay">
          ...
          <el-col :md="8" :sm="12" :xl="6" :xs="24">
            <el-form-item label="交易状态：">
              <el-select v-model="formInline.tradeStatus" clearable placeholder="全部">
                <el-option v-for="item in tradeStatusArray" :key="item.id" :label="item.name" :value="item.id"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :md="8" :sm="12" :xl="6" :xs="24">
            <el-form-item label="交易类型：">
              <el-select v-model="formInline.tradeType" clearable placeholder="全部">
                <el-option v-for="item in tradeTypeArray" :key="item.id" :label="item.name" :value="item.id"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :md="8" :sm="12" :xl="6" :xs="24">
    ...
    <div ref="tableContentRef" class="tableContent content_border">
      <TableHeaderTitle title="占位订单交易明细">
        <template #content>
          <OrderTotalAndExportCom :returnDataInfo="returnDataInfo"></OrderTotalAndExportCom>
        </template>
      </TableHeaderTitle>
      <div ref="tableCenterRef" v-resize="setTableMaxHeight" class="tableCenter">
        <el-table v-loading="listLoading" :data="list" :max-height="tableMaxHeight" stripe ref="tableRef">
          ...
          <el-table-column align="center" fixed="left" label="订单号">
            <template #default="{ row }">{{ $filters.moreData(row.orderNum) }}</template>
          </el-table-column>
          <el-table-column align="center" label="交易流水号" show-overflow-tooltip>
          ...
          <el-table-column align="center" label="金额（元）">
            <template #default="{ row }">
              <div class="flex-jc-ai-center">
                <div :class="{burdenMoney: row.tradeMoney < 0}" class="tradeMoney">
                  <span class="symbol">{{ row.tradeMoney < 0 ? '-' : '+' }}</span>
          ...
          <el-table-column align="center" label="交易状态">
            <template #default="{ row }">
              <div class="flex-jc-ai-center">
                <div :class="'tradeStatus' + row.tradeStatus" class="tradeStatus">{{$filters.tradeStatus(row.tradeStatus)}}</div>
              </div>
            </template>
          </el-table-column>
    ...
    <OrderDetailsDialog v-if="orderDetailsVisible" v-model:isVisible="orderDetailsVisible" :activeOrderId="activeOrderId"></OrderDetailsDialog>
  </div>
</template>
...
import OrderDetailsDialog from "./ChargingOrderDetailsDialog.vue";
...
export default defineComponent({
  name: "CsTransactionOccupancyOrder",
  ...
  setup() {
    ...
    const that = reactive({
      ...
      tradeType: [{id: 1, name: "占位收入"}, {id: 2, name: "占位退款"}],
    ...
    const queryTenantAccountList = ()=>{
      let formInline = JSON.parse(JSON.stringify(that.formInline));
      findAccountListByUserId({ platformType: formInline.payPlatform,tenantId: userInfo.value.tenantId }).then(res=>{
    ...
    return {...toRefs(that),tableRef, listArray, clickResetForm, tableContentRef, tablePaginationRef, setTableMaxHeight, queryTenantAccountList, userInfo, clickOperateBut};
  }
});
</script>
...
.tableContent {
  ...
  .tradeStatus1,.burdenMoney{
    color: #FD944A;
  }

  .tradeStatus2,.tradeMoney{
    color: #56E540;
  }
}
</style>