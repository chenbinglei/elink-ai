<template>
  <div class="app-container-right">
        ...
        <el-row :gutter="16">
          ...
          <el-col :md="8" :sm="12" :xl="6" :xs="24">
            <el-form-item label="商户平台：">
              <el-select v-model="formInline.tradeWay" filterable clearable placeholder="全部"
                @change="queryTenantAccountList">
          ...
          <el-col :md="8" :sm="12" :xl="6" :xs="24">
            <el-form-item label="企业商户：">
              <el-select v-model="formInline.accountId" :disabled="!formInline.tradeWay" clearable placeholder="全部">
    ...
    <div ref="tableContentRef" class="tableContent content_border">
      <TableHeaderTitle title="V2G钱包交易明细">
        <template #content>
          <OrderTotalAndExportCom componentName="CsTransactionV2GWalletOrder" :formInline="formInline">
      ...
      <div ref="tableCenterRef" v-resize="setTableMaxHeight" class="tableCenter">
        <el-table v-loading="listLoading" :data="list" :max-height="tableMaxHeight" stripe ref="tableRef">
          ...
          <el-table-column align="center" fixed="left" label="订单号" show-overflow-tooltip>
            <template #default="{ row }">{{ $filters.moreData(row.orderNum) }}</template>
          </el-table-column>
          <el-table-column align="center" label="交易类型">
            <template #default="{ row }">{{ $filters.tradeV2GType(row.tradeType) }}</template>
          </el-table-column>
          <el-table-column align="center" label="金额（元）">
            <template #default="{ row }">
              <div class="flex-jc-ai-center">
                <div :class="{ burdenMoney: row.tradeType !== 1}" class="tradeMoney">
                  <span class="symbol">{{ row.tradeType !== 1 ? '-' : '+' }}</span>
          ...
          <el-table-column align="center" label="企业商户">
    ...
    <V2GOrderDetailsDialog v-if="orderDetailsVisible" v-model:isVisible="orderDetailsVisible"
...
import V2GOrderDetailsDialog from "./V2GOrderDetailsDialog.vue";
...
import { onMounted, reactive, ref, toRefs, defineComponent, computed, getCurrentInstance, nextTick } from "vue";
import { queryDischargeTradeList } from "@/api/operationManagement/CsTransactionDetails";
import { findAccountListByUserId } from "@/api/operationManagement/CsSettlementManagement";
import { useRoute } from 'vue-router';
export default defineComponent({
  name: "CsTransactionV2GWalletOrder",
  props: {
    routeInfo: {
      type: Object,
      default: () => {
  ...
  components: { OrderTotalAndExportCom, V2GOrderDetailsDialog },
  setup (props) {
    ...
    const that = reactive({
      ...
      tradeTypeArray: [{ id: 1, name: "V2G收益存入" }, { id: 2, name: "余额提现" }],
      keywordTypeArray: [{ id: 1, name: "订单号" }, { id: 2, name: "手机号" }, { id: 3, name: "站点名称" }],
    ...
    const listArray = (operateType) => {
      ...
      console.log(formInline,route.query,'****');
      ...
      // if (route.query) {
      //   console.log('查询参数999:', route.query)
      //   that.formInline.keywordType = 2;
      //   that.formInline.keyword = route.query.mchId;
      //   // that.formInline.accountId = route.query.accountId;


      // }
      queryDischargeTradeList({ page: that.currentPage, size: that.pageNum, ...formInline }).then(async res => {
    ...
    return {
      ...toRefs(that), tableRef, listArray, clickResetForm, tableContentRef, tablePaginationRef, setTableMaxHeight, queryTenantAccountList, userInfo,
      clickOperateBut
    };
  }
});
