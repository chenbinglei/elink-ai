<template>
  <div class="app-container">
    <div class="app-container-right">
          ...
          <el-row :gutter="16">
            ...
            <el-col :md="8" :sm="12" :xl="6" :xs="24">
              <el-form-item label="开始时间：">
                <el-date-picker v-model="activeDeviceInfo.dataTime" :disabled-date="pickerOptions.disabledDate"
      ...
      <!-- <div id="map" style="height: 500px; width:100%;background:white"></div> -->
      ...
      <div class="tableContent content_border" ref="tableContentRef">
        ...
        <div class="tableCenter" ref="tableCenterRef" v-resize="setTableMaxHeight">
          <el-table v-loading="listLoading" :data="InvoiceList" :max-height="tableMaxHeight"
            @selection-change="handleSelectionChange" class="custom-expand-table" @expand-change="handleExpandChange"
            ref="tableRef">
            <!-- 展开行 - 放置在选择框之前 -->
            <el-table-column type="expand" width="80">
              <template #default="scope">
                <div class="expand-content">
                  <div v-if="latitudelongitude.length !== 0" class="map-wrapper">
                    <div :id="'map-container-' + scope.row.id" class="map-container" v-loading="maploading"></div>
                    <div class="map-table" ref="tableContentRef" v-resize="setTableMaxHeights">
                      <el-table :data="latitudelongitude" border stripe :max-height="tableMaxHeights">
            ...
            <el-table-column prop="userName" label="当前处理人" show-overflow-tooltip></el-table-column>
              ...
              <template #default="scope">
                <div class="table_operate_class">
                  <el-link :underline="'never'"
                    v-if="scope.row.taskStatus === 2 || scope.row.taskStatus === 3 || scope.row.taskStatus === 5"
                    @click="clickDetails(scope.row)">详情</el-link>
                  <el-link :underline="'never'" v-if="scope.row.taskStatus === 1"
                    @click="clickDetails(scope.row)">执行</el-link>
                  <el-link :underline="'never'" v-if="scope.row.taskStatus === 4"
                    @click="clickDetails(scope.row)">验收</el-link>
                </div>
              </template>
    ...
    <!-- 新增 -->
    <addTaskDialog v-if="addTaskVisible" v-model:isVisible="addTaskVisible" @saveDialog="saveDialog"
      @queryList="queryList" />
    <nodePersonDialog v-if="nodePersonVisible" v-model:isVisible="nodePersonVisible" @saveDialog="saveDialog" />
    <!-- 详情 -->
    <detailsDialog :inspectionType="inspectionType" :inspectionArray="inspectionArray" v-if="detailsVisible"
      v-model:isVisible="detailsVisible" @saveDialog="saveDialog" />
...
const queryList = () => {
  ...
  delete obj.dataTime;
  queryInspectionTaskList(obj).then((res) => {
...
const clickResetForm = () => {
  ...
  currentPage.value = 1
  pageNum.value = 10
  pickerOptions.value.taskName = ''
  pickerOptions.value.taskStatus = ''
...
const handleExpandChange = (row, expanded) => {
  console.log('handleExpandChange', row, expanded.length);
  if (expanded.length) {
    // 行已展开，可以加载地图
    console.log('行已展开，可以加载地图', row);
    findInspectionTaskDetailById({ id: row.id }).then(res => {
      latitudelongitude.value = res.data.inspectionSiteList
    })
    nextTick(() => {
      initMap(row)
    })
...
//   }).then((AMap) => {
//     const coords = latitudelongitude.value.map(item => {
//       const lon = Number(item.longitude);
//       const lat = Number(item.latitude);
//       return [lon, lat];
//     });
//     const map = new AMap.Map(`map-container-${row.id}`, {
//       zoom: 8,
//       center: coords[0] // 北京坐标
//     });
...
const initMap = (row) => {
  ...
  // 如果已有地图实例，先销毁
  if (mapInstances.value.has(row.id)) {
    const map = mapInstances.value.get(row.id)
    map.destroy()
    mapInstances.value.delete(row.id)
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkContainer = () => {
        ...
        if (container) {
          ...
          reject(new Error(`地图容器 ${containerId} 未找到`))
        }
      }
  ...
  ensureContainerReady().then(() => {
    AMapLoader.load({
      ...
      // 获取坐标数据
      const coords = latitudelongitude.value
      ...
      console.log('初始化地图，坐标数量:', coords.length)
...
const clickDetails = (row) => {
  ...
  console.log(inspectionArray.value, 'row')
}
