<template>
    <div v-loading="loading"
        ...
        <div class="h-220px flex-shrink-0 flex justify-between items-stretch mb-6px">
            <!-- 设备详情 -->
            <DeviceInfo title="光伏逆变器" icon="photovoltaicInverters" avatar="inverter" class="w-769px flex-shrink-0"
                :info="deviceInfoData" />
            <!-- 遥控遥调 -->
        ...
        <div class="h-312px flex-grow-1  flex-shrink-0  flex items-stretch justify-between overflow-hidden">
            <BlockChart type-id="3" class="w-769px  flex-shrink-0" />
            <BlockChart type-id="4" class="w-769px  flex-shrink-0" />
        </div>
    </div>
</template>
...
const { loading, deviceInfoData } = useDeviceData({ ...props, apiName: 'findPvInverterData' });
</script>
