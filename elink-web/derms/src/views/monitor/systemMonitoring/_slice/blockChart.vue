<template>
    <MonitorBlock :title="title" :icon="icon">
        <template v-if="tabList?.length > 1" #extra>
            <FancyTab :tabs="tabList" v-model:activeTab="activeTabId" @update:activeTab="onTabChange" />
        </template>
        <template #content>
            <div class="w-full h-full overflow-hidden relative" v-loading="loading">
                <div class="absolute right-25px top-14px z-2">
                    <el-date-picker v-model="date" v-bind="pickerProps" v-on="pickerEvents" v-if="!hidePicker" />
                ...
                <v-chart ref="myChart" :option="chartOptions" v-bind="commonVChartProps" />
            </div>
        </template>
    </MonitorBlock>
</template>
...
const {
    title, icon,
    //tab
    tabList, activeTabId, onTabChange,
    //日期
    hidePicker, date, pickerProps, pickerEvents,
    //chart
    loading,myChart, chartOptions, onExport
} = useBlockChart({
    ...props,
});
