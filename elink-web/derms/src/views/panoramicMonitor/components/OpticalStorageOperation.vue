  ...
  <div class="optical-storage-operation">
    <div class="optical-storage-operation-title w-full flex jc-space-between">
      <div class="real-time-power-title flex-ai-center">
        <img src="@/assets/image/panoramic-monitor/p-m-header-icon5.png" alt="" width="20" />
        <span class="ml-2">光储运营</span>
      </div>
      <TabGroup class="right-tab" :tab-active="activeTab" :tab-list="tabList" @tabChangeEvent="tabChangeHandler">
      </TabGroup>
    </div>
    <div class="gfzfdl flex">
      <div class="gfzfdl-icon">
        <img src="@/assets/image/panoramic-monitor/icon-gfcn.png" alt="" width="95" v-if="activeTab == 0" />
        <img src="@/assets/image/panoramic-monitor/icon-gfcn2.png" alt="" width="95" v-else />
      </div>
      <div class="gfzfdl-text flex jc-center" v-if="activeTab == 0">
        <p>光伏总发电量</p>
        <div class="gfzfdl-value">
          <span class="special-text ml-5">{{ $filters.moreZero(pvOperation.totalQt) }}</span>kWh
        </div>
      </div>
      <div class="gfzfdl-text flex jc-center" v-if="activeTab == 1">
        <p>储能总充/放电量</p>
        <div class="gfzfdl-value">
          <span class="special-text ml-5">{{$filters.moreZero( storageOperation.chargeTotalQt) }}/{{
    ...
    <!-- <div class="details">
      <template v-for="(citem, index) in topTotal[activeTab]">
        <div class="details-item flex" style="width: calc((100% - 20px) / 2);height: 20%"
          :class="index == 0 ? '' : 'pl-5'">
          ...
          <div class="ml-2 pt-2">
            ...
            <div class="details-state flex mt-3" :style="backgroundStyle(
              activeTab == 0
                ? pvOperation[citem.jiaosyProp]
                : storageOperation[citem.jiaosyProp]
            )
              ">
              较上月<span :class="getClass(
                activeTab == 0
                  ? pvOperation[citem.jiaosyProp]
                  : storageOperation[citem.jiaosyProp]
              )
    ...
    <div class="gcyy" v-if="activeTab == 0">
      <div class="flex">
        <div class="gcyy-icon">
          <img src="@/assets/image/panoramic-monitor/icon-gcyy-3.png" alt="" width="29" />
        </div>
        <div class="w-full ml-2">
          <div class="gcyy-top flex jc-space-between">
            <div class="gcyy-top-item" :class="item.className" v-for="(item, index) in bottomTotal[0]"
              :key="'topTotal' + index">
              <div class="gcyy-top-text">{{ item.label }}</div>
              <div class="gcyy-top-value">
                <span class="special-text">{{$filters.moreZero( pvOperation[item.numProp] )}}</span>
                {{ item.unit }}
              </div>
            </div>
          </div>
    ...
    <div v-if="activeTab == 1" class="flex jc-space-between pl-5 pr-5 gcyy">
      <template v-for="(item, index) in bottomTotal[1]">
        <div class="flex ai-center fd-column">
          <div style="padding: 13px 0">
            <img :src="item.img" alt="" class="storage-icon" />
          </div>
          <div class="text-white">
            {{ item.label }}
            <span class="special-text ml-2 mr-2" :class="item.className">{{
              $filters.moreZero(storageOperation[item.numProp])
            }}</span>
            {{ item.unit }}
          </div>
        </div>
    ...
    <div class="chart-content">
      <EchartsTwo ref="echart1" width="100%" height="100%" font-size="12" :data="chartData"></EchartsTwo>
    </div>
    <div class="gcyy-bottom flex jc-space-between" v-if="activeTab == 0">
      <div class="gcyy-bottom-item flex">
        <div>
          <img src="@/assets/image/panoramic-monitor/icon-gcyy-4.png" alt="" width="29" />
        </div>
        <div class="ml-2">
          <div class="gcyy-bottom-value special-text">
            <span>
              <!-- {{ pvOperation.standardCoalReduction }} -->
                {{ pvOperation.standardCoalReduction >= 1000
                ? $filters.moreZero(`${(pvOperation.standardCoalReduction / 1000).toFixed(4)}`)
                : $filters.moreZero(`${pvOperation.standardCoalReduction}` )}}
            </span>
            <!-- t -->
          </div>
          <div class="gcyy-bottom-text">节约标煤{{ pvOperation.standardCoalReduction >= 1000
                ? '(t)':"(kg)" }}</div>
        </div>
      </div>
      ...
      <div class="gcyy-bottom-item flex">
        ...
        <div class="ml-2">
          <div class="gcyy-bottom-value special-text">
            ...
            <span>
              {{ pvOperation.co2Reduction >= 1000
                ?$filters.moreZero(`${(pvOperation.co2Reduction / 1000).toFixed(4)}`)
                : $filters.moreZero(`${pvOperation.co2Reduction}`) }}
            </span>
            <!-- t -->
          </div>
          <div class="gcyy-bottom-text">CO2减排 {{ pvOperation.co2Reduction >= 1000
                ? '(t)':"(kg)" }}</div>
        </div>
      </div>
      <div class="gcyy-bottom-line"></div>
      <div class="gcyy-bottom-item flex">
        ...
        <div class="ml-2">
          <div class="gcyy-bottom-value special-text">
            <span>{{$filters.moreZero(pvOperation.treeReduction)  }}</span>棵
          </div>
