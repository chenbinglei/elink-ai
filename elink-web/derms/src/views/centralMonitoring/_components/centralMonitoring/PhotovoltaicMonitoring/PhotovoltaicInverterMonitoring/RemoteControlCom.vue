<template>
  <TitleView :isTitleIcon="false" :title="titleName" is-content-height>
    <template #content>
      ...
      <PowerSettingDialog v-if="powerSettingVisible" :isVisible="powerSettingVisible" :operateType="operateType" :titleName="dialogTitleName"/>
    </template>
  </TitleView>
</template>
...
export default defineComponent({
  ...
  components: {ControlModelDialog, PowerSettingDialog},
  props: {
    titleName: {
      type: String,
      default: ""
  ...
  setup() {
    const that = reactive({
      ...
      list: [{id: 1, name: "控制模式"}, {id: 2, name: "有功功率"}, {id: 3, name: "无功功率"}]
    });
