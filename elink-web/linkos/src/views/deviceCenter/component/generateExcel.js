...
/*
	...
	* @param tableHeader 中文 英文 表头
	* @param {String} fileName 导出Excel文件名字
    * @param {String} sheetName 导出sheetName名字
*/
export async function exportPointTableExcel(tableHeader = [], json = [], fileName = "文件名称", sheetName = "Sheet1") {
    const loading = ElLoading.service({lock: true, text: '正在下载中...', background: 'rgba(0, 0, 0, 0.7)'});

    const workbook = new ExcelJS.Workbook(); //创建工作簿
    const worksheet = workbook.addWorksheet(sheetName, {
        views: [{state: 'frozen', xSplit: 4, ySplit: 0}], // 冻结列
...
// 设备管理==批量导入模板
export async function exportDeviceCustomExcel(tableHeader = [], json = [], fileName = "文件名称", sheetName = "Sheet1") {
    const loading = ElLoading.service({lock: true, text: '正在下载中...', background: 'rgba(0, 0, 0, 0.7)'});
    const workbook = new ExcelJS.Workbook(); //创建工作簿
    const worksheet = workbook.addWorksheet(sheetName, {
        properties: {defaultColWidth: 18} // 默认列宽
