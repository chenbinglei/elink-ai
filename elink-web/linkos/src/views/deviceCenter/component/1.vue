  ...
  setup (props) {
    ...
    function removeNodeByData (nodes, data) {
      return nodes.filter(node => {
        ...
        const isMatch = node.id === data.id && node.parentId === data.parentId;
    ...
    const handleMenuEvent = (data) => {
      if (data.type === "clickTreeNode") {
        ...
        const nodesWithChildren = selectedNodes
          .filter(node => 'children' in node)
          .map(node => ({ ...node, children: [] })); // 不改变原数据
        // 遍历 nodesWithChildren 和 selectedNodes，查找 id == parentId 的节点
        nodesWithChildren.forEach(parentNode => {
          const matchingChildren = selectedNodes?.filter(childNode =>
            parentNode.id == childNode.parentId
          );

          if (matchingChildren.length > 0) {
            parentNode.children.push(...matchingChildren);
          }
        });
        that.checkMenuNum = nodesWithChildren.reduce((sum, node) => sum + node.children.length, 0);
        that.checkMenuArray = setTreeData(nodesWithChildren)
      }
      if (data.menuType === "clickClosedIcon") {
        ...
        // handleMenusLeftRef.value.setCheckedKeysFun(selectIdArray);
        // let levelTreeArray = treeToArray(selectIdArray); // 转为一维数组
        // let checkMenuIdsArray = levelTreeArray.map(item => item.id); // 提取所有 id
        let levelTreeArray = treeToArray(selectIdArray); // 转为一维数组
        console.log("handleMenuEvent", selectIdArray, that.checkMenuArray,levelTreeArray);
    ...
    //     that.checkMenuNum = data.allSelectIdArray.length;
    ...
    // }
    // const handleMenuEvent = (data) => {
    //   console.log("handleMenuEvent",that.handleMenuArray);
    //   console.log(data);
    //   if (data.type === "clickTreeNode") {
    //     let checkMenuArray = [], deviceArray = [];
    //     that.checkMenuNum = data.allSelectIdArray.length;
    //     for (let i = 0; i < data.allSelectIdArray.length; i++) {
    //       let item = that.handleMenuArray.find(item => item.id === data.allSelectIdArray[i]);
    //       if(item){
    //         checkMenuArray.push(item);
    //       }
    //     }
    //     that.checkMenuArray = setTreeData(checkMenuArray)
    //     console.log(checkMenuArray,'checkMenuArray');
    //   }
    ...
    const initParamConfigFun = () => {
      ...
      findSubDeviceFunctionListByDeviceId({ deviceId: props.activeDeviceId, type: 1, pageName: "ImportPointTable" }).then(res => {
        ...
        for (let i = 0; i < functionArray.length; i++) {
          ...
          functionArray[i].id = functionArray[i].functionId;
    ...
    return {
      ...
      handleMenuEvent, handleMenusLeftRef, removeNodeByData, downloadTemplate
    }
  }
})
</script>
