  ...
  <div class="modelDynamicFieldCom">
    <el-form ref="formDialogRef" :model="formDialog" :rules="rules" label-width="150px">
      <el-row :gutter="10">
        ...
        <template v-if="(modelFieldArray && modelFieldArray.length) || scenarioType !== 0">
          <template v-for="(item,index) in modelFieldArray" :key="index">
            <!--                位置-->
                ...
                <el-form-item :label="item.reaName + '：'" :required="item.required">
                  ...
                  <el-col :span="3">
                    <div class="flex jc-end">
                      <el-button type="primary" :icon="Location" @click="clickCoordinatePicking(1,item.fieldName)">坐标拾取</el-button>
                    </div>
                  </el-col>
                </el-form-item>
              </el-col>
            ...
            <el-col :span="12" style="margin-bottom: 18px" v-else>
...
import {getModelFieldUpdateListByModelId} from "@/api/deviceCenter/deviceList";
import LocationAndRegionDialog from "@/components/component/LocationAndRegionDialog";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // 根据模型id获取模型编辑字段列表
    const queryModelFieldUpdateListByModelId = ()=>{
      getModelFieldUpdateListByModelId({ modelId: props.modelId,timer: new Date() }).then(res=>{
        let newModelFieldArray = [];
        let modelFieldArray = res.data ? res.data : [];
        for(let i = 0;i < modelFieldArray.length;i++){
          if(modelFieldArray[i].type === 2){

            if(modelFieldArray[i].fieldName && !that.rules[modelFieldArray[i].fieldName]){
              if(modelFieldArray[i].required){
                
                // 位置信息字段校验方式
                if(modelFieldArray[i].reaType === 4){
                  that.rules[modelFieldArray[i].fieldName + '.longitude'] = locationInfoVerificationFun(modelFieldArray[i],'经度');
                  that.rules[modelFieldArray[i].fieldName + '.latitude'] = locationInfoVerificationFun(modelFieldArray[i],'纬度');
                  that.rules[modelFieldArray[i].fieldName + '.address'] = locationInfoVerificationFun(modelFieldArray[i],'详细地址');
                } else {
                  // 默认字段校验方式
                  that.rules[modelFieldArray[i].fieldName] = [{
                    trigger: "change",
                    required: modelFieldArray[i].required,
                    validator: (rule, value, callback)=>{
                      if ((!value && value !== 0) || (hasWhiteSpace(value) && modelFieldArray[i].reaType !== 6) ) {
                        callback(new Error(`请选择或输入正确的${ modelFieldArray[i].reaName }`));
                      } else {
                        callback();
                      }
                    }
            ...
            // 新建需要进行赋值操作
            if(!props.newEditStatus){
              // 默认字段回填
              if((modelFieldArray[i].defaultValue || modelFieldArray[i].defaultValue === 0)){
                that.formDialog[modelFieldArray[i].fieldName] = modelFieldArray[i].defaultValue;
              }
            }

            // 位置信息字段
            if(modelFieldArray[i].reaType === 4 && !that.formDialog[modelFieldArray[i].fieldName]){
              that.formDialog[modelFieldArray[i].fieldName] = {};
            }

            // 额外属性解析
            modelFieldArray[i].extraValue = modelFieldArray[i].extraValue ? JSON.parse(modelFieldArray[i].extraValue) : {};
            newModelFieldArray.push(modelFieldArray[i]);
          }
        }

        // console.log(newModelFieldArray);
        that.modelFieldArray = JSON.parse(JSON.stringify(newModelFieldArray));
        that.listLoading = false;
      }).catch(()=>{
        that.listLoading = false;
        clearFormDialogValueFun();
      })
    }
    ...
    const clearFormDialogValueFun = ()=>{
      if(that.modelFieldArray && that.modelFieldArray.length){
        for(let i = 0;i < that.modelFieldArray.length;i++){
          if(that.modelFieldArray[i].fieldName){
            delete that.rules[that.modelFieldArray[i].fieldName];
            delete that.formDialog[that.modelFieldArray[i].fieldName];
          }
        }
      }
      that.modelFieldArray = [];
    }
    ...
    const setDatePickTypeFun = (extraValue)=>{
      let dateType = "date";
      if(extraValue.timeFormat.indexOf('HH') !== -1)dateType = "datetime";
      return  dateType
    }
    ...
    const setDatePickFormatFun = (extraValue)=>{
      ...
      if(extraValue.timeFormat.indexOf('HH') !== -1){
        dateType = "datetime";
    ...
    const watchModelId = watch(()=>props.modelId,(newModelId)=>{
      if(newModelId) queryModelFieldUpdateListByModelId();
    },{ deep: true })

    onMounted(()=>{
      if(props.modelId) queryModelFieldUpdateListByModelId();
    })

    return {...toRefs(that),queryModelFieldUpdateListByModelId, clearFormDialogValueFun, watchModelId, clickCoordinatePicking, mapEvents, setDatePickTypeFun,
      setDatePickFormatFun, formDialogRef, formSubmitFun, queryAreaAddressByCoordinates, locationInfoVerificationFun}
  }
})
</script>
