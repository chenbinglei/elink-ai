    ...
    <div class="bottomContent">
      ...
      <div class="list_menu">
        <div class="list_menu_content scrollbarStyle">
          <el-tree ref="treeBoxRef" :current-node-key="currentLivingId" :data="list"
            :default-checked-keys="defaultCheckedKeys" :default-expand-all="defaultExpandAll"
            :default-expanded-keys="defaultExpandedKeys" :filter-node-method="filterTreeNode"
            :highlight-current="!show_checkbox" :indent="12" :props="treeProps" :show-checkbox="show_checkbox"
            :expand-on-click-node="expandOnClickNode" auto-expand-parent check-on-click-node
            class="rightArrowClass tree-line" node-key="id" @check-change="checkChangeFun" @node-click="clickTreeNode">
            <template #default="{ node, data }">
              <div class="custom-tree-node">
                <div class="left_content">
                  <div v-if="data.iconName" :class="[data.iconName, 'iconfont']"></div>
                  <div class="label textTwo" v-html="node.label"></div>
                  <div v-if="displayClosed" class="iconfont icon-chahao" @click.stop="clickClosedIcon(data)"></div>
                </div>
              </div>
            </template>
          </el-tree>
...
export default defineComponent({
  ...
  setup (props) {
    ...
    const checkChangeFun = () => {
      if (that.show_checkbox) {
        const selectedIds = treeBoxRef.value.getCheckedKeys();
        const selectedNodes = treeBoxRef.value.getCheckedNodes(false, true); // 获取所有选中节点（包括子节点）

        emit("handleMenuEvent", { type: "clickTreeNode", allSelectIdArray: selectedIds, selectedNodes });
      }
    };
    // 点击某一条数据后的叉号
    const clickClosedIcon = (data) => {
      emit("handleMenuEvent", { menuType: "clickClosedIcon", activeClickId: data });
    }
    ...
    const setCheckedKeysFun = (checkedIdArray = []) => {
      console.log(checkedIdArray,'checkedIdArray','点击删除按钮')
    ...
    return {
      ...toRefs(that), extractIds, flattenTree, listLengthFun, getChildrenFirstFun, clickTreeNode, treeBoxRef, watchArray, setTreeSelectCheck, searchInput, clickRightBut,
