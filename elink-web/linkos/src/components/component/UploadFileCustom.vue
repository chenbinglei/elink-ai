<template>
  <div class="uploadFileCustom" :class="[class_file_type === 1 ? 'uploadImageClass' : 'uploadFileClass']">
    <el-upload ref="uploadRef" v-model:file-list="fileListArray" :auto-upload="false" :limit="fileLength" :disabled="isDisabled" :on-change="uploadChange"
               :drag="class_file_type === 2" class="upload_class" :on-exceed="exceedFile" :accept="acceptType" :on-remove="handleRemove">
      <template #tip>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const uploadChange = (file, fileList) => {
      ...
      nextTick(()=>emit("changEvent", { type: "uploadChange",fieldName: props.fieldName }));
    }
    ...
    return {...toRefs(that), exceedFile, uploadRef, uploadChange, handleRemove, watchFileListArray, watchFileArray}
  }
})
</script>
