<template>
  <div class="uploadPicturesCom">
    <el-upload ref="uploadRef" v-model:file-list="fileListArray" :auto-upload="false" :limit="fileLength" :disabled="isDisabled" :on-change="uploadChange"
               :on-exceed="exceedFile" accept=".jpg, .jpeg, .png, .JPG" class="upload_class" list-type="picture-card"
               :class="{ showAddImageIcon: fileListArray.length >= fileLength }">
...
import {Delete, Plus, Minus, Search} from '@element-plus/icons-vue';
import {defineComponent, getCurrentInstance, nextTick, onMounted, reactive, ref, toRefs, watch} from "vue";

export default defineComponent({
  name: "UploadPicturesCom",
  components: {Viewer, Search, Plus, Delete, Minus},
  props: {
    // 上传张数
    uploadNum: {
      type: Number,
