<template>
  <div class="uploadFileCustom">
    <el-upload ref="uploadRef" v-model:file-list="fileListArray" :auto-upload="false" :limit="fileLength"
      :disabled="isDisabled" :on-change="uploadChange" class="upload_class" :on-exceed="exceedFile" :accept="acceptType"
      :on-remove="handleRemove">
        <el-button type="primary">{{ fileName }}</el-button>
      <template #tip>
        <div class="el-upload__tip">
          <slot name="tip_content"></slot>
        </div>
      </template>
      <!-- <template #default> <el-button type="primary">Click to upload</el-button></template> -->
    </el-upload>
  </div>
</template>
