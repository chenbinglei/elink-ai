  ...
  <div class="handleMenu">
    ...
    <div v-if="isShowHeader" class="title">
      ...
      <div class="rightTopButIcon">
        <span v-if="showAddButton" class="rightTopButImg iconfont icon-treeAdd" @click="clickRightBut"></span>
      ...
      <div class="list_menu">
        <div class="list_menu_content scrollbarStyle">
          <el-tree ref="treeBoxRef" :current-node-key="currentLivingId" :data="list" :default-checked-keys="defaultCheckedKeys"
                   ...
                   :expand-on-click-node="expandOnClickNode" auto-expand-parent check-on-click-node class="rightArrowClass tree-line"
                   node-key="id" @check-change="checkChangeFun" @node-click="clickTreeNode">
...
export default defineComponent({
  ...
  props: {
    ...
    // 数据更新是否重新获取第一项  false: 获取  true: 不获取
    dataRenewIsFun: {
      type: Boolean,
      default: false
    },
    // 数据更新 或者 默认 都不进行 重新选择数据，（需手动选中）
    defaultIsChecked: {
      type: Boolean,
      default: false
    },
  ...
  setup(props) {
    ...
    const checkChangeFun = () => {
      ...
      nextTick(() => {
        ...
        emit("handleMenuEvent", {menuType: "clickTreeNode", allSelectIdArray: selectIdArray});
      })
    }
    ...
    const setCheckedKeysFun = (checkedIdArray = []) => {
      ...
      if(props.showCheckbox){
        treeBoxRef.value.setCheckedKeys(checkedIdArray);
        emit("handleMenuEvent", {menuType: "clickTreeNode", allSelectIdArray: that.defaultCheckedKeys });
      } else {
        // console.log(checkedIdArray);
        that.currentLivingId = checkedIdArray[0];
        treeBoxRef.value.setCurrentKey(checkedIdArray[0]);
        emit("handleMenuEvent", {menuType: "clickTreeNode", id: checkedIdArray[0] });
      }
    }
    ...
    const watchArray = watch([() => props.handleMenuArray, () => props.showCheckbox], ([newHandleMenuArray, newShowCheckbox]) => {
      ...
      if(!props.defaultIsChecked){
        // console.log("重新设置选中数据！！！");
        if (props.dataRenewIsFun || that.firstPage || !that.currentLivingId || (!props.dataRenewIsFun && JSON.stringify(that.oldAllList).indexOf(that.currentLivingId) === -1)) {
          ...
          nextTick(()=> emit("handleMenuEvent", { type: "initHandleMenu",msg: "数据更新完成！" }));
        } else {
          nextTick(()=>treeBoxRef.value.setCurrentKey(that.currentLivingId));  // 单选
        }
      }
    }, {deep: true});

    onMounted(() => {
      if(!props.defaultIsChecked) listLengthFun();
    });
