...
const service = axios.create({
    ...
    baseURL: requestPath,  // api的base_url
...
service.interceptors.request.use((config) => {
    ...
    // 打包注释
    // if (config.portNum) {
        //     config.baseURL = config.baseURL.replace("158","142");
        // config.baseURL = config.baseURL.replace(portNum, `:${config.portNum}`);
    // }

    if (!config.noLoginRequired) {
        ...
        if (config.data instanceof FormData) {
            ...
            if (getToken()) {
                ...
                // if(config.baseURL.indexOf("142") === -1) config.data.append("access_token", getToken());
            }
        } else {
            ...
            if (getToken()) {
                ...
                // if(config.baseURL.indexOf("142") === -1) config.data.access_token = getToken();
            }
        }
