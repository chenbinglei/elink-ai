...
export function setTreeData(data = [], parentId = "parentId") {
...
* analysis: 解构对象中某个数据
*  */
export function selectTreeData(value = "", key = "id", arr = [], analysis = false) {
    let newArr = [];
    arr.forEach(element => {
        // 把对象中某个值合并到主对象中
...
export function getFileNameFromPath(path) {
    let FIlE_NAME = path.split('/').pop().split('\\').pop();
...
export function queryUserAuthorityIsHaveFun(pathName = "") {
    ...
    if (route_list && route_list.length) {
        for (let i = 0; i < route_list.length; i++) {
            if (pathName && pathName.indexOf(route_list[i].router_path) !== -1) {
                isAuthority = true;
                break
            }
        }
...
export function setThreeData(arrayData = []) {
    if (arrayData && arrayData.length) {
        for (let i = 0; i < arrayData.length; i++) {
            arrayData[i].id = arrayData[i].name;
            arrayData[i].label = arrayData[i].meta.title;
            if (arrayData[i].children && arrayData[i].children.length) setThreeData(arrayData[i].children);
        }
    }
...
// Excel表格解析成JSON
export function readWorkbookFromLocalFile(files) {
    let respondBody = {};
    ...
    return new Promise(function (resolve, reject) {
        fileReader.onload = function (ev) {
            try {
                ...
                respondBody = {code: 2000, msg: "文件解析成功", body: sheetContent};
                resolve(respondBody);
            } catch (e) {
                respondBody = {code: 500, msg: "文件类型不正确", body: []};
                reject(respondBody);
            }
        };
    });
