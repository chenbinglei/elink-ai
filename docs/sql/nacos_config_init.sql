-- Nacos MySQL 持久化初始化脚本
-- 用途：为Nacos从Derby切换到MySQL创建必要的数据库和表
-- 使用方式：mysql -h ${MYSQL_HOST} -u root -p < nacos_config_init.sql
-- 创建日期：2026-06-05
...
CREATE TABLE IF NOT EXISTS `config_info` (
  ...
  `encrypted_data_key` varchar(1024) DEFAULT '' COMMENT '密钥',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `config_info_beta` (
  ...
  `content` longtext NOT NULL COMMENT 'content',
  `beta_ips` longtext COMMENT 'betaIps',
  `md5` varchar(32) DEFAULT NULL COMMENT 'md5',
  `gmt_create` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `gmt_modified` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
  `src_user` text COMMENT 'source user',
...
CREATE TABLE IF NOT EXISTS `group_capacity` (
  ...
  `group_id` varchar(128) NOT NULL DEFAULT '' COMMENT 'Group ID，空表示整个集群',
...
CREATE TABLE IF NOT EXISTS `his_config_info` (
  ...
  `tenant_id` varchar(128) DEFAULT '' COMMENT '租户字段',
  `encrypted_data_key` varchar(1024) DEFAULT '' COMMENT '密钥',
  `publish_type` varchar(20) DEFAULT 'formal' COMMENT 'publish type gray or formal',
  `ext_info` longtext COMMENT 'ext info',
  PRIMARY KEY (`nid`),
